        ...
        {flores.map((f, i) => (
          <div key={i} className="group relative rounded-[16px] overflow-hidden aspect-square flex flex-col justify-end p-5 md:p-8 cursor-pointer animate-fade-up" style={{ animationDelay: `${i * 0.1}s` }}>
            ...
            <div className="relative z-10 flex flex-col">
              <span className="font-display text-[1.4rem] md:text-[1.8rem] font-bold text-[var(--color-background-primary)] leading-none mb-1 md:mb-2">{f.name}</span>
              <span className="text-[0.7rem] md:text-[0.8rem] text-[var(--color-background-primary)]/70 uppercase tracking-[0.1em] font-medium">{f.sub}</span>
            </div>
          </div>
        ))}
      </div>
