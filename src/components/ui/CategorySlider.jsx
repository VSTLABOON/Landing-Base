...
export default function CategorySlider({ items, isOpen, categoryTitle, id }) {
  ...
  const handleCotizar = (e, item) => {
    ...
    if (!cleanNumber || cleanNumber === '0000000000' || cleanNumber.length < 10) {
      return; // Silently skip if no real number configured
    }
    const msg = encodeURIComponent(
      `Hola! Me interesa cotizar *${item.name}* (${item.priceRange}) de la categoría "${categoryTitle}". ¿Tienen disponibilidad?`
    );
    window.open(`https://wa.me/${cleanNumber}?text=${msg}`, '_blank', 'noopener');
  };
      ...
      <div className="overflow-hidden">
        <div className="px-8 md:px-10 pt-8 pb-10 border-t border-white/5 bg-negro/50">
          ...
          <div className="flex items-center justify-between mb-6">
            <h4 className="text-[var(--color-background-primary)]/90 font-display text-[1.2rem] md:text-[1.4rem] font-bold flex items-center gap-3">
              Catálogo de {categoryTitle}
              <span className="hidden sm:block h-[1px] w-16 bg-[var(--color-background-primary)]/10"></span>
              <span className="text-[0.7rem] font-body font-normal text-[var(--color-background-primary)]/30 tracking-[0.1em] uppercase hidden sm:inline">
            ...
            {items.length > 2 && (
              <div className="flex items-center gap-2">
                <button
                  ...
                  aria-label="Anterior"
                  className="w-9 h-9 rounded-full border border-white/10 bg-[var(--color-background-primary)]/5 flex items-center justify-center text-[var(--color-background-primary)]/60 transition-all duration-200 hover:bg-[var(--color-background-primary)]/10 hover:text-[var(--color-background-primary)] disabled:opacity-20 disabled:cursor-not-allowed disabled:hover:bg-[var(--color-background-primary)]/5"
                >
            ...
            {items.map((item, idx) => (
              ...
              >
                <div className="relative w-full aspect-[3/4] rounded-[16px] overflow-hidden mb-4 bg-verde-dark shadow-[0_4px_20px_rgba(0,0,0,0.3)] transition-shadow duration-300 group-hover/card:shadow-[0_8px_32px_rgba(0,0,0,0.5)]">
                  ...
                  <div className="absolute inset-0 flex items-center justify-center opacity-0 group-hover/card:opacity-100 transition-all duration-300">
                    <button
                      ...
                      className="bg-rosa hover:bg-rosa-light text-[var(--color-background-primary)] text-[0.72rem] font-bold uppercase tracking-[0.12em] px-6 py-2.5 rounded-full transform translate-y-6 group-hover/card:translate-y-0 transition-all duration-400 ease-spring shadow-[0_4px_16px_rgba(217,79,110,0.4)] focus:outline-none focus-visible:ring-2 focus-visible:ring-white"
                    >
                ...
                <div className="flex flex-col px-1">
                  <span className="text-[var(--color-background-primary)] font-semibold text-[1rem] leading-tight mb-1 group-hover/card:text-rosa transition-colors duration-200">
                    {item.name}
                  </span>
                  <span className="text-[var(--color-background-primary)]/40 text-[0.75rem] tracking-[0.04em]">
                    {categoryTitle}
                  </span>
                </div>
              </div>
              ...
              {items.map((_, idx) => (
                <button
                  ...
                  className={`rounded-full transition-all duration-300 ${
                    idx === activeIndex
                      ? 'w-6 h-2 bg-rosa'
                      : 'w-2 h-2 bg-[var(--color-background-primary)]/20 hover:bg-[var(--color-background-primary)]/40'
                  }`}
